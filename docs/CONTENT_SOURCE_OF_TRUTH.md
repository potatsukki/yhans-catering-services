# Content Source of Truth

Use this file as the factual source of truth. Do not invent prices, inclusions, policies, menus, testimonials, or capacities.

## Business identity

- Official business name: **Yhan's Catering Services**
- Facebook display name: **Yhan's Catering Service Official**
- Owner: **Marianne P. Natanawan**
- Role: **Chef / Proprietor**
- Established: **2010**
- Tagline: **Making Every Celebration Delicious**
- Registration: **DTI and BIR registered**
- Guest capacity for catered events: **50 to 600 guests**

## Contact information

- Phone / Viber 1: **0956 675 5148**
- Phone / Viber 2: **0967 119 5792**
- Email: **marianne03natanawan@gmail.com**
- Facebook URL: **https://www.facebook.com/share/1EnpK8EnM1/**
- Address:
  **Block 19, Lot 11, Dahlia Extension Street, BIR Village, West Fairview, Quezon City 1118, Philippines**
- Business response hours: **Daily, 8:00 AM–7:00 PM**

### Visit location and food tasting

- Display hours for the location section: **Daily, 8:00 AM–7:00 PM**
- Google Maps coordinates: **14.6995033, 121.0536876**
- Official Google Maps page: **https://www.google.com/maps/place/Natanawan+Residence/@14.6995046,121.0530439,19z/data=!3m1!4b1!4m6!3m5!1s0x3397b100609ab79b:0x8660534f6a53f2a!8m2!3d14.6995033!4d121.0536876!16s%2Fg%2F11ybc4ythr?entry=ttu&g_ep=EgoyMDI2MDcyOS4wIKXMDSoASAFQAw%3D%3D**
- Visit note: **Please message or call before visiting so Yhan's Catering Services can confirm availability.**
- Food tasting: Customers may request a complimentary tasting of selected menu items before confirming an event. Tastings are subject to availability; Marianne P. Natanawan arranges and confirms the final schedule.

Do not promise a tasting request will be approved, imply that a tasting is included with every booking, promise every requested dish will be available, or allow customers to select or reserve their own tasting date or time.

## Payment methods

- Cash
- Bank transfer
- GCash

Do not publish bank account numbers or GCash account details.

## About story

Approved public wording:

> Established in 2010, Yhan's Catering Services started with a simple setup and limited equipment. Through years of experience, hard work, and continuous investment, the business grew into a more professional catering service with improved equipment, presentation, hygiene standards, and event setup. Today, Yhan's Catering Services serves private celebrations, business functions, and group events with dependable food and service.

Do not claim an exact investment amount. “Continuous investment” is sufficient.

### Founder profile

- Founder and owner: **Marianne P. Natanawan**
- Public role: **Chef / Proprietor**
- Owner-confirmed framing: Marianne built Yhan's Catering Services from humble beginnings, starting with the simple setup and limited equipment described above and leading its growth through hard work, hands-on experience, and continuous investment.
- Approved emphasis: she has led **Yhan's Catering Services** toward improved equipment, presentation, hygiene standards, event setup, and dependable service for private celebrations, business functions, and group events.

Do not add awards, rankings, celebrity claims, exact investment figures, culinary credentials, or biographical details that the owner has not confirmed.

## Events served

Confirmed event types:

- Weddings
- Debuts
- Baptisms / christenings
- Graduations
- Seminars
- Induction ceremonies
- Corporate events
- Family gatherings
- Other celebrations and group events involving food service

## Service areas

Primary areas:

- Quezon City
- Caloocan
- Manila
- Nearby Metro Manila areas

Approved wording for farther locations:

> Farther locations may be accommodated. Transportation, travel, accommodation, staffing, and other event arrangements will be discussed directly based on the location and event requirements.

Do not publish fixed out-of-area, transport, overtime, or accommodation prices.

## Full catering capacity

- Minimum normal booking: **50 guests**
- Published maximum: **600 guests**

Do not publish 650 as the normal maximum.

---

# Regular Catering Packages

## Pricing model

- Regular Packages 01, 02, and 03 each cost **₱30,000**
- Each package is **good for 50 guests**
- Present this as a fixed package, not as a per-head price
- Do not display “₱600 per head”
- Pricing for more than 50 guests is not confirmed

Approved overflow wording:

> Planning for more than 50 guests? Message Yhan's Catering Services for a custom quotation.

## Confirmed shared inclusions

These inclusions were stated for the regular packages:

- One dessert
- Salad bar
- Iced tea
- Rice
- Soup
- Tables and chairs
- Buffet setup

Serving staff has not yet been reconfirmed as a public package inclusion. Do not list waiter or waitress service as confirmed until the owner confirms it.

## Regular Package 01

- Price: **₱30,000**
- Capacity: **Good for 50 guests**
- Menudo
- Chopsuey
- Chicken Cordon Bleu
- Beef with Mushroom
- Spaghetti

## Regular Package 02

- Price: **₱30,000**
- Capacity: **Good for 50 guests**
- Beef Caldereta
- Pork Hamonado
- Chopsuey
- Lumpiang Shanghai
- Spaghetti

## Regular Package 03

- Price: **₱30,000**
- Capacity: **Good for 50 guests**
- Beef Caldereta
- Buffalo Chicken
- Lumpiang Shanghai
- Menudo
- Spaghetti

---

# Catering Packages With Grazing Table

## Pricing model

- Price: **₱1,000 per guest**
- Minimum: **50 guests**
- Total depends on confirmed guest count
- Tables and chairs are included

Examples for internal testing only:

- 50 guests = ₱50,000
- 75 guests = ₱75,000
- 100 guests = ₱100,000
- 150 guests = ₱150,000

A small guest-count estimator may be included. It must clearly say that it is an estimate and that final details are confirmed directly.

## Grazing Package A

Main menu:

- Beef Kare-Kare
- Pork Menudo
- Chicken Wings
- Fish with Tofu
- Egg Drop Soup
- Rice
- Spaghetti

## Grazing Package B

Main menu:

- Beef Caldereta
- Pork Menudo — Best Seller
- Buffalo Chicken
- Fish with Creamy Sauce
- Chicken Chowder Soup
- Rice
- Pasta Carbonara

## Grazing Package C

Main menu:

- Beef with Mushroom
- Pork Menudo
- Chicken Cordon Bleu
- Baked Bangus
- Potato Soup
- Rice
- Chicken Pesto

Do not rename “Chicken Pesto” to “Chicken Pesto Pasta” unless the owner confirms that it is pasta.

## Grazing Package D

Main menu:

- Beef with Broccoli
- Bicol Express
- Chicken Afritada
- Sweet and Sour Fish
- Pumpkin Soup
- Rice
- Pancit Canton

## Shared grazing-table inclusions

### Fruits

- Seasonal fruits
- Watermelon
- Grapes
- Oranges

### Fresh items

- Lettuce
- Cucumber

### Desserts and sweets

- Mango Tapioca
- Coffee Jelly
- Cake Bites
- Cupcakes
- Cookies
- Crinkles

### Savory items

- Chicken Empanada
- Lumpiang Shanghai
- Nori Chicken
- Brie Cheese
- Camembert Cheese
- Salami
- Bacon
- Bite-Sized Bread

### Beverages

- Lemon Iced Tea
- Water

---

# Packed Meals / Food Packs

Food packs are separate from full catering packages.

## Purpose

Suitable for:

- breakfast, lunch, or dinner;
- group orders;
- business and office orders;
- call center accounts;
- meetings;
- seminars;
- school or community activities.

## Pricing

- **₱250 to ₱300 per pack / person**
- The final price depends on menu, quantity, and requirements

Do not invent bulk discounts.

## Confirmed sample menus

### Breakfast sample

- Rice
- Egg
- Hotdog

### Lunch sample

- One piece of chicken
- Vegetables
- Pancit

### Dinner

Dinner is available, but the exact menu is not confirmed. Show “Menu options available upon request.”

## Operations and documents

- The catering team assists in preparing, portioning, and repacking food packs.
- Large corporate orders may require business documents such as DTI and BIR registration.
- A 400-person call-center order was given as an example of a large order, not as a fixed maximum.

## Pending food-pack details

Do not invent:

- minimum order quantity;
- delivery fee;
- whether drinks are included;
- whether utensils and tissue are included;
- exact dinner choices;
- exact lead time;
- maximum daily capacity;
- bulk discounts.

Use:

> Contact us for available menus, minimum quantities, delivery arrangements, and a custom bulk quotation.

---

# Food Trays

Food trays are offered separately from full catering.

Confirmed:

- Customers can order selected dishes without full on-site catering.
- Trays were discussed as approximately good for 25 to 30 people, but exact servings depend on the dish.
- A categorized food-tray menu exists or will be provided later.

Not yet confirmed:

- complete dish list;
- prices;
- exact serving sizes;
- minimum number of trays;
- delivery and pickup rules.

Public website wording:

> Food trays are available for parties, offices, and group meals. Message us to request the current food-tray menu, serving sizes, and prices.

Do not display invented tray prices.

---

# Menu customization

Approved wording:

> Selected dishes may be replaced with another option from the same food category. Premium substitutions, additional dishes, and special requests may have additional charges and must be discussed directly.

Do not promise that every substitution is free.

---

# Additional Event Services

Yhan's Catering Services may help arrange these through contacts or trusted partners:

- Event styling
- Venue decoration
- Backdrop
- Sound system
- Event host
- Photographer
- Videographer
- Other event-related suppliers

Approved wording:

> Additional event services may be arranged through trusted partners. Availability, scope, and pricing will be discussed directly based on your event requirements.

Do not publish fixed partner prices.

---

# Booking and payment process

Confirmed steps:

1. Customer contacts the business through Facebook, phone, Viber, or email.
2. Customer discusses date, location, guest count, package, and event requirements.
3. Details are confirmed through a catering contract.
4. A **70% down payment** is required to reserve the date.
5. The remaining balance must be paid **three days before the event**.

Approved policy wording:

> Cancellation, rescheduling, refund, and other booking conditions are governed by the signed catering contract.

Do not invent cancellation or refund terms.

---

# Calls to action

There is no website form.

Use these actions:

- `Message Us on Facebook`
- `Request a Quote`
- `Call Us Today`
- `Send an Email`
- `View Packages`
- `Ask for the Food Tray Menu`
- `Discuss Your Event`

Behavior:

- Facebook buttons open the Facebook page in a new tab.
- Email buttons use `mailto:marianne03natanawan@gmail.com`.
- Phone buttons use `tel:+639566755148`.
- Secondary phone may use `tel:+639671195792`.
- Add accessible labels explaining that external links open in a new tab.

---

# Placeholder policy

For prototype imagery:

- use licensed or project-provided placeholder images;
- prefer local assets;
- provide descriptive alt text;
- centralize asset references;
- lazy-load below-the-fold images;
- never imply that a stock image is a real Yhan's Catering event.

For testimonials:

- owner-approved customer profiles and feedback wording may be displayed;
- Glydel Anne Dabu — School Event: “The food was delicious, and the service helped our school event run smoothly. Thank you for making the occasion feel special.”
- Yhen Natanawan — BPO Company: “The team handled our company food service efficiently, and everything was prepared and presented well for our group.”
- Jonathan Camara — Wedding: “The catering helped make our wedding celebration warm and memorable. Our guests enjoyed the food and the beautiful setup.”
- Alexandra Alarcon — Birthday Celebration: “The food, presentation, and service made the birthday celebration easy to enjoy with family and friends.”
- do not add dates, venues, ratings, or additional event details unless the owner confirms them.
